module Combat
  module Abilities
    # Death nuke dealing TRUE damage — ignores armor, resist, absorb shields, AND
    # type advantage by writing straight to health. Scales on magic_power so it
    # grows into a reliable anti-tank / anti-shield answer as the caster gears up
    # (stacks with other power sources for real depth).
    class SoulCrush < Base
      ...
      def resolve(caster:, state:, tick:)
        ...
        target.take_damage(dmg, bypass_barrier: true) # true damage punches through shields
        ...
        events = [TimelineEvent.new(
          ...
          target_shield_after: target.barrier, damage_type: "true"
        )]
