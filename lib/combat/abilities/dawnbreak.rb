module Combat
  module Abilities
    ...
    # ally within one tile of the caster by raising their shield stat
    # (shield_scale * magic_power) for the rest of the battle. The type circle
      ...
      def resolve(caster:, state:, tick:)
        ...
        victims.sort_by(&:id).each do |victim|
          ...
          events << TimelineEvent.new(
            ...
            damage: dmg, target_health_after: victim.current_health, damage_type: "magic"
          )
          events << TimelineEvent.new(tick: tick, type: "death", unit_id: victim.id) unless victim.alive?
        end

        # Ward: allies adjacent to the caster gain shield (magic mitigation).
